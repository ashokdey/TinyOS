...
LMEntry:
    ...
    ; by this time the kernel is aleady loaded 
    ; we will relocate the kernel to mem addr - 200k
    
    ; first of all we will clear the direction 
    ; so the MOV instruction can mov the data from 
    ; low mem addr to high mem addr 
    ; i.e data is moved in forward direction 
    CLD
    MOV rdi, 0x200000
    MOV rsi, 0x10000

    ; register rcx will act as a counter 
    ; the instruction will copy 8 byte data each time 
    ; when we load the kernel into the memory, 
    ; we read 100 sectors and each sector is 512 bytes 
    MOV rcx, 51200/8

    ; in order to repeat the MOV ins, we use rep movsq
    REP MOVSQ

    ; kernel is now copied to mem arrd 200,000
    JMP 0x200000
